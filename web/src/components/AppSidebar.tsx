...
const NAV_LINKS = [
  ...
  { href: '/chat', icon: 'chat', label: 'Agent Chat' },
          ...
          {NAV_LINKS.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-2.5 px-2.5 py-2 text-[13px] font-mono transition-colors relative group ${
        ...
        <div className={`flex ${collapsed ? 'flex-col' : ''} gap-1`}>
          {THEMES.map((t) => (
            <button
              ...
              title={t.label}
              className={`${collapsed ? '' : 'flex-1'} border p-1.5 text-center transition-colors ${
                theme === t.id
                  ? 'border-poly-accent bg-poly-accent text-poly-accent-text'
                  : 'border-poly-border text-poly-muted hover:text-poly-text'
              }`}
            >
              <span className={`font-mono uppercase tracking-wider font-bold ${collapsed ? 'text-[8px]' : 'text-[9px]'}`}>
